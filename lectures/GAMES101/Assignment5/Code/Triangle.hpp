...
bool rayTriangleIntersect(const Vector3f &v0, const Vector3f &v1,
    ...
    Vector3f E0(v0 - v2), E1(v1 - v2), S(orig - v2);
    Vector3f P = crossProduct(E0, dir);
    float det = dotProduct(E1, P);
    if (det < 0) {
        return false;
    }
    float invDet = 1.0f / det;
    v = invDet * dotProduct(S, P);
    if (v < 0 || v > 1.0f) {
        return false;
    }

    Vector3f Q = crossProduct(E1, S);
    u = invDet * dotProduct(dir, Q);
    if (u < 0 || u + v > 1.0f) {
        return false;
    }
    tnear = invDet * dotProduct(E0, Q);
    if (tnear < 0) {
        return false;
    }
    return true;
