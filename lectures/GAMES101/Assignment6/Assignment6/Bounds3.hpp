...
inline bool Bounds3::IntersectP(const Ray &ray, const Vector3f &invDir,
    ...
    Vector3f cMin = pMin - ray.origin;
    Vector3f cMax = pMax - ray.origin;
    float tIn = std::numeric_limits<float>::min();
    float tOut = std::numeric_limits<float>::max();
    for (int i = 0; i < 3; i++) {
        float curIn = cMin[i] * invDir[i];
        float curOut = cMax[i] * invDir[i];
        if (dirIsNeg[i]) {
            std::swap(curIn, curOut);
        }
        tIn = std::max(curIn, tIn);
        tOut = std::min(curOut, tOut);
        // fprintf(stderr, "cMin: %f, cMax: %f, tIn: %f, tOut: %f\n", cMin[i], cMax[i],tIn,tOut);
        if (tOut < 0) {
            return false;
        }
    }
    return tOut >= 0 and (tIn < tOut);
}
